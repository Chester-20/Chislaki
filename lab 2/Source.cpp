...
vector<double> f(vector<double> x) {
    vector<double> F;
    F.push_back(log(1 + (x[0] + x[1]) / 5) - sin(x[1] / 3) - x[0] + 1.1);
    F.push_back(cos(x[0] * x[1] / 6) - x[1] + 0.5);
    return F;
}

void jac(double x1, double x2, vector<vector<double>>& jacobi) {
    jacobi[0][0] = - (x1 + x2 + 4) / (x1 + x2 + 5);
    jacobi[0][1] = 1 / (x2 + x1 + 5) - cos(x2 / 3) / 3;
    jacobi[1][0] = - x2 * sin(x1 * x2) / 2;
    jacobi[1][1] = -x1 * sin(x1 * x2) / 2 - 1;
}
...
int main() {
    ...
    newtonWithMyJacobi(1, 1);
    cout << endl;
    cout << "M = 0.01" << endl;
    newtonWithGeneralJacobi(1, 1,0.01);
    cout << endl;
    cout << "M = 0.05" << endl;
    newtonWithGeneralJacobi(1, 1, 0.05);
    cout << endl;
    cout << "M = 0.1" << endl;
    newtonWithGeneralJacobi(1, 1, 0.1);
}
